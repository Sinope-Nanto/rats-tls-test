...
extern tls_wrapper_err_t nulltls_use_privkey(tls_wrapper_ctx_t *ctx, rats_tls_cert_algo_t algo,
					     void *privkey_buf, size_t privkey_len);
extern tls_wrapper_err_t nulltls_use_cert(tls_wrapper_ctx_t *ctx, rats_tls_cert_info_t *cert_info);
...
#endif
{
	...
	if (err != TLS_WRAPPER_ERR_NONE)
		RTLS_ERR("failed to register the tls wrapper 'nulltls' %#x\n", err);
}
